...
func (pq *PlaylistQuery) QuerySongs() *MusicQuery {
...
func (pq *PlaylistQuery) WithSongs(opts ...func(*MusicQuery)) *PlaylistQuery {
	query := (&MusicClient{config: pq.config}).Query()
...
func (pq *PlaylistQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Playlist, error) {
	...
	if query := pq.withSongs; query != nil {
		if err := pq.loadSongs(ctx, query, nodes,
			func(n *Playlist) { n.Edges.Songs = []*Music{} },
			func(n *Playlist, e *Music) { n.Edges.Songs = append(n.Edges.Songs, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (pq *PlaylistQuery) loadSongs(ctx context.Context, query *MusicQuery, nodes []*Playlist, init func(*Playlist), assign func(*Playlist, *Music)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(playlist.SongsTable)
		s.Join(joinT).On(s.C(music.FieldID), joinT.C(playlist.SongsPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(playlist.SongsPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(playlist.SongsPrimaryKey[0]))
	...
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "songs" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
